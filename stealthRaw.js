(() => {
    ...
    (function inject2() {
        const changeCanvasMain = function () {
            ...
            rsalt = random([...Array(7).keys()].map(a => a - 3))
            gsalt = random([...Array(7).keys()].map(a => a - 3))
            bsalt = random([...Array(7).keys()].map(a => a - 3))
            asalt = random([...Array(7).keys()].map(a => a - 3))
                ...
                apply: function (target, ctx, args) {
                    ...
                    for (let i = 0; i < height; i++) {
                        for (let j = 0; j < width; j++) {
                            ...
                            console.log(rsalt, gsalt, bsalt, asalt)
